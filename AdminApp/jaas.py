...
def generate_jaas_jwt(staff, room_name, moderator=False):
    ...
    In production (Render), the private key is read from
    JAAS_PRIVATE_KEY environment variable.

    Locally, it can fall back to JAAS_PRIVATE_KEY_PATH.
    """

    # ---------------------------------------------------------
    # Get JaaS private key
    # ---------------------------------------------------------
    private_key = os.getenv("JAAS_PRIVATE_KEY")

    # Local fallback: read from .pem file
    if not private_key:
        private_key_path = os.getenv(
            "JAAS_PRIVATE_KEY_PATH",
            "jaas-private-key.pem"
